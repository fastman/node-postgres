...
var parseConnectionString = function(str) {
  ...
  if (result.query) {
      for (var param in result.query) {
          if (result.query.hasOwnProperty(param)) {
              config[param] = result.query[param];
          }
      }
  }
  return config;
...
var normalizeConnectionInfo = function(config) {
  ...
  case 'object':
    ...
    norm(config, 'keepalives_idle');
    norm(config, 'keepalives_interval');
    norm(config, 'keepalives_count');
    return config;
  case 'string':
...
var getLibpgConString = function(config, callback) {
  if(typeof config == 'object') {
    ...
    add(params, config, 'keepalives_idle');
    add(params, config, 'keepalives_interval');
    add(params, config, 'keepalives_count');
    if(config.database) {
